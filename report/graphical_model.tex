...
&= p(\theta|\alpha) p(\varphi|\beta)\prod^N_{i=1}\prod^{M_i}_{j=1} p(z_{i,j}|\theta_{y_i}) p(w_{i,j}|\varphi_{z_{i,j}})\\
P(\varphi | \beta) &= \prod\limits_{k=1}^K P(\varphi_k|\beta)\\
P(\theta | \alpha) &= \prod\limits_{g=1}^G P(\theta_g|\alpha)\\
P(z | \theta, y) &= \prod\limits_{i=1}^N \prod\limits_{j=1}^{M_i} P(z_{i,j}|\theta_g(z_{i,j}), y_i)\\
